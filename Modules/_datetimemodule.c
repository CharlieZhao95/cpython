...
// TODO(zhaoyu64@huawei.com): check function name, and
// maybe force convert `state` to `_datetimemodule_state` is insecure.
#define _PyType_GET_DATETIME_MODULE_STATE(op) \
    ((_datetimemodule_state *)PyType_GetModuleState(Py_TYPE(op)))

#define PyDate_Check(op) PyObject_TypeCheck(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_DateType)
#define PyDate_CheckExact(op) Py_IS_TYPE(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_DateType)

#define PyDateTime_Check(op) PyObject_TypeCheck(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_DateTimeType)
#define PyDateTime_CheckExact(op) Py_IS_TYPE(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_DateTimeType)

#define PyTime_Check(op) PyObject_TypeCheck(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_TimeType)
#define PyTime_CheckExact(op) Py_IS_TYPE(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_TimeType)

#define PyDelta_Check(op) PyObject_TypeCheck(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_DeltaType)
#define PyDelta_CheckExact(op) Py_IS_TYPE(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_DeltaType)

#define PyTZInfo_Check(op) PyObject_TypeCheck(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_TZInfoType)
#define PyTZInfo_CheckExact(op) Py_IS_TYPE(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_TZInfoType)

#define PyTimezone_Check(op) PyObject_TypeCheck(op, _PyType_GET_DATETIME_MODULE_STATE(op)->PyDateTime_TimeZoneType)
...
{
    PyObject *result;
    _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(cls));
...
create_timezone(PyObject *offset, PyObject *name)
{
    ...
    _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(offset));
...
{
    _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(p));
...
multiply_int_timedelta(PyObject *intobj, PyDateTime_Delta *delta)
{
    ...
    PyObject *pyus_out;
    PyObject *result;
    _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(delta));
...
delta_add(PyObject *left, PyObject *right)
{
    ...
    if (PyDelta_Check(left) && PyDelta_Check(right)) {
        ...
        _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(left));
...
{
    // TODO(zhaoyu64@huawei.com): maybe PyType_GetModuleState() could be replaced by macro
    return new_delta(-GET_TD_DAYS(self),
                     -GET_TD_SECONDS(self),
                     -GET_TD_MICROSECONDS(self),
                     1, ((_datetimemodule_state *)PyType_GetModuleState(Py_TYPE(self))));
}
...
delta_positive(PyDateTime_Delta *self)
{
    ...
    return new_delta(GET_TD_DAYS(self),
                     GET_TD_SECONDS(self),
                     GET_TD_MICROSECONDS(self),
                     0, ((_datetimemodule_state *)PyType_GetModuleState(Py_TYPE(self))));
}
...
delta_remainder(PyObject *left, PyObject *right)
{
    ...
    remainder = microseconds_to_delta(pyus_remainder, state);
...
delta_divmod(PyObject *left, PyObject *right)
{
    ...
    delta = microseconds_to_delta(PyTuple_GET_ITEM(divmod, 1), state);
    if (delta == NULL) {
        Py_DECREF(divmod);
        return NULL;
    }
    ...
    if (PyDate_Check(left)) {
        if (PyDate_Check(right)) {
            ...
            int right_ord = ymd_to_ord(GET_YEAR(right),
                                       ...
                                       GET_DAY(right));
            _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(left));
            return new_delta(left_ord - right_ord, 0, 0, 0, state);
        }
        if (PyDelta_Check(right)) {
            /* date - delta */
            return add_date_timedelta((PyDateTime_Date *) left,
...
{
    _datetimemodule_state *state = PyType_GetModuleState(Py_TYPE(self));
...
time_hash(PyDateTime_Time *self)
{
    if (self->hashcode == -1) {
        ...
        if (offset == Py_None)
            ...
            temp1 = new_delta(0, seconds, microseconds, 1, state);
            if (temp1 == NULL) {
                Py_DECREF(offset);
                return -1;
            }
...
datetime_astimezone(PyDateTime_DateTime *self, _datetimemodule_state *state, PyObject *args, PyObject *kw)
{
...
datetime_getdate(PyDateTime_DateTime *self, PyObject *Py_UNUSED(ignored))
{
    return new_date(GET_YEAR(self),
                    GET_MONTH(self),
                    GET_DAY(self),
                    ((_datetimemodule_state *)PyType_GetModuleState(Py_TYPE(self))) );
}
...
_datetime_exec(PyObject *module)
{
    ...
    state->PyDateTime_DateType = PyType_FromModuleAndSpec(module, &PyDateTime_DateType_spec, NULL);
    state->PyDateTime_DeltaType = PyType_FromModuleAndSpec(module, &PyDateTime_DeltaType_spec, NULL);
    state->PyDateTime_TimeType = PyType_FromModuleAndSpec(module, &PyDateTime_TimeType_spec, NULL);
    state->PyDateTime_TZInfoType =PyType_FromModuleAndSpec(module, &PyDateTime_TZInfoType_spec, NULL);
    state->PyDateTime_IsoCalendarDateType = PyType_FromModuleAndSpec(module, &PyDateTime_IsoCalendarDateType_spec, NULL);
    state->PyDateTime_TimeZoneType = PyType_FromModuleAndSpec(module, &PyDateTime_TimeZoneType_spec, NULL);
    state->PyDateTime_DateTimeType = PyType_FromModuleAndSpec(module, &PyDateTime_DateTimeType_spec, NULL);

    // `&...` is not a constant expression according to a strict reading
    // of C standards. Fill tp_base at run-time rather than statically.
    // See https://bugs.python.org/issue40777

    // set specific base class by PyType_FromModuleAndSpec?
