...
create_timezone(PyObject *offset, PyObject *name)
{
    ...
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(offset);
...
{
    if (p == Py_None || PyTZInfo_Check(p, get_datetimemodule_state_iterinst(p)))
        return 0;
...
get_tzinfo_member(PyObject *self)
{
    ...
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(self);
...
{
    PyObject *offset;
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(tzinfo);
...
make_freplacement(PyObject *object)
{
    ...
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(object);
...
multiply_int_timedelta(PyObject *intobj, PyDateTime_Delta *delta)
{
    ...
    PyObject *pyus_out;
    PyObject *result;
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(delta);
...
delta_add(PyObject *left, PyObject *right)
{
    ...
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(left);
...
delta_negative(PyDateTime_Delta *self)
{
    return new_delta(-GET_TD_DAYS(self),
                     ...
                     1, get_datetimemodule_state_iterinst(self));
}
...
delta_positive(PyDateTime_Delta *self)
{
    ...
    return new_delta(GET_TD_DAYS(self),
                     GET_TD_SECONDS(self),
                     GET_TD_MICROSECONDS(self),
                     0, get_datetimemodule_state_iterinst(self));
}
...
delta_subtract(PyObject *left, PyObject *right)
{
    ...
    if (PyDelta_Check(left, state) && PyDelta_Check(right, state)) {
        ...
        int microseconds = GET_TD_MICROSECONDS(left) -
                           GET_TD_MICROSECONDS(right);
        result = new_delta(days, seconds, microseconds, 1, get_datetimemodule_state_iterinst(left));
    }
    ...
    if (PyDelta_Check(left, state)) {
        if (PyDelta_Check(right, state))
            result = truedivide_timedelta_timedelta(
                            (PyDateTime_Delta *)left,
                            (PyDateTime_Delta *)right);
        else if (PyFloat_Check(right, state))
            result = multiply_truedivide_timedelta_float(
                            (PyDateTime_Delta *)left, right, 1);
        else if (PyLong_Check(right, state))
...
delta_remainder(PyObject *left, PyObject *right)
{
    ...
    remainder = microseconds_to_delta(pyus_remainder, get_datetimemodule_state_iterinst(left));
...
delta_divmod(PyObject *left, PyObject *right)
{
    ...
    delta = microseconds_to_delta(PyTuple_GET_ITEM(divmod, 1), get_datetimemodule_state_iterinst(left));
    if (delta == NULL) {
        Py_DECREF(divmod);
        return NULL;
    }
    ...
    if (PyDate_Check(left, state)) {
        if (PyDate_Check(right, state)) {
            ...
            int right_ord = ymd_to_ord(GET_YEAR(right),
                                       GET_MONTH(right),
                                       GET_DAY(right));
            return new_delta(left_ord - right_ord, 0, 0, 0, get_datetimemodule_state_iterinst(left));
        }
        if (PyDelta_Check(right, state)) {
            /* date - delta */
            return add_date_timedelta((PyDateTime_Date *) left,
...
{
    _datetimemodule_state *state = get_datetimemodule_state_iterinst(dt);
...
time_hash(PyDateTime_Time *self)
{
    if (self->hashcode == -1) {
        ...
        if (offset == Py_None)
            ...
            temp1 = new_delta(0, seconds, microseconds, 1, get_datetimemodule_state_iterinst(self));
            if (temp1 == NULL) {
                Py_DECREF(offset);
                return -1;
            }
...
time_fromisoformat(PyObject *cls, PyObject *tstr) {
    ...
    const char *p = PyUnicode_AsUTF8AndSize(tstr, &len);
     _datetimemodule_state *state = get_datetimemodule_state_iterinst(cls);
...
datetime_getdate(PyDateTime_DateTime *self, PyObject *Py_UNUSED(ignored))
{
    return new_date(GET_YEAR(self),
                    GET_MONTH(self),
                    GET_DAY(self),
                    get_datetimemodule_state_iterinst(self));
}
